...
namespace HoloToolkit.Unity.Buttons
{
    ...
    public class CompoundButtonMesh : ProfileButtonBase<ButtonMeshProfile>
    {
        ...
        //Kliment's code
        //the original color of the material of the button when not being pressed
        private Color originalColor, pressedColor;
        ...
        protected void Start ()
        {
            ...
            //kliment
            originalColor = transform.GetChild(0).gameObject.GetComponent<MeshRenderer>().material.color;
                //the 5 buttons that generate the proteins
            if(gameObject.GetComponent<DataLoader>() != null)
            {
                pressedColor = GetComponent<DataLoader>().proteinPrefab.GetComponent<ColorProperty>().color;
            }
                //other buttons regarding atoms to visualize
            else if(gameObject.GetComponent<Atom_Selector>()!=null)
            {
                Invoke("SetPressedColor", 1.3f);
            }
            ...
            if(gameObject.name == "ResetProtein")
            {
                gameObject.tag = "ResetButton";
            }
            //
        }
        ...
        protected void Update ()
        {
            ...
            //Kliment's code
            //change the color of the button accordingly
            if(GetComponent<ToggleProperties>()!=null && GetComponent<ToggleProperties>().isHighlighted)
            {
                transform.GetChild(0).gameObject.GetComponent<MeshRenderer>().material.color = pressedColor;
            }
            else
            {
                transform.GetChild(0).gameObject.GetComponent<MeshRenderer>().material.color = originalColor;
            }
        }
        ...
        {
            pressedColor = GetComponent<Atom_Selector>().currentProtein.GetComponent<ColorProperty>().color;
        }
