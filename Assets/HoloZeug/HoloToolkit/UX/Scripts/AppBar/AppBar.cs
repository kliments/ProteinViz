        ...
        private void Update()
        {
            ...
            switch (State)
            {
                case AppBarStateEnum.Default:
                    ...
                    //kliment code
                    targetBarSize = new Vector3(6, 1.5f, 1);
                    backgroundBar.transform.localPosition = Vector3.Lerp(backgroundBar.transform.localPosition, new Vector3(-0.0627f, 0, 0.0145f), 0.5f);
                    if(!atomsMenu.activeSelf)
                    {
                        atomsMenu.SetActive(true);
                    }
                    //
                    break;
                ...
                case AppBarStateEnum.Hidden:
                    ...
                    //kliment
                    backgroundBar.transform.localPosition = Vector3.Lerp(backgroundBar.transform.localPosition, new Vector3(0,0,0), 0.5f);
                    if(atomsMenu.activeSelf)
                    {
                        atomsMenu.SetActive(false);
                    }
                    break;

                case AppBarStateEnum.Manipulation:
                    targetBarSize = new Vector3((numManipulationButtons-1) * buttonWidth, buttonWidth, 1f);
                    //kliment
                    backgroundBar.transform.localPosition = Vector3.Lerp(backgroundBar.transform.localPosition, new Vector3(-0.06f, 0, 0), 0.5f);
                    if (atomsMenu.activeSelf)
                    {
                        atomsMenu.SetActive(false);
                    }
